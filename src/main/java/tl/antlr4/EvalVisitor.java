...
public class EvalVisitor extends TLBaseVisitor<TLValue> {
    ...
    public TLValue multiply(MultExpressionContext ctx) {
        ...
        if (lhs.isNumber() && rhs.isNumber()) {
            if (lhs.isInteger() && rhs.isInteger()) {
                return new TLValue(lhs.asInteger() * rhs.asInteger());
            } else {
    ...
    public TLValue visitAssignment(AssignmentContext ctx) {
        ...
        if (ctx.indexes() != null) {
            ...
            System.out.println("Inside" + ctx.Identifier().getText());
            List<ExpressionContext> exps = ctx.indexes().expression();
    ...
    public TLValue visitRepeatStatement(RepeatStatementContext ctx) {
        for (int i = 0; i < Integer.parseInt(String.valueOf(ctx.Number())); i++) {
            TLValue returnValue = this.visit(ctx.block());
            if (returnValue != TLValue.VOID) {
                return returnValue;
            }
    ...
    public TLValue visitSpeakStatement(SpeakStatementContext context) {
        if (context.expression() != null) {
            ...
            String statement = this.visit(context.expression()).toString();
            freettsVoice.speak(statement);
    ...
    public TLValue visitErrorExpression(ErrorExpressionContext ctx) {
        throw new EvalException("Check the quotation at line number: ", ctx);
    }
}
